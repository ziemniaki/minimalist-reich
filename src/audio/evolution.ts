import { Part, Time, Transport, Frequency, Sampler } from 'tone';
...
export class Voice {
  ...
  constructor(public sampler: Sampler, seed: number[], dur: string) {
    this.seed = [...seed];
    this.ost = { pattern: [...seed], dur, mutateRate: 0.2, density: 1 };
    this.part = new Part((time, note: number) => {
      const freq = Frequency(note, 'midi').toFrequency();
      this.sampler.triggerAttackRelease(freq, this.ost.dur, time);
    }, [] as any[]).start(0) as any;
    this.part.loop = true;
  }
  ...
  reschedule(root: number) {
    this.part.clear();
    const step = Time(this.ost.dur).toSeconds();
    this.ost.pattern.forEach((st, i) => {
      if (Math.random() < this.ost.density) {
        this.part.add(i * step, root + st);
      }
    });
    this.part.loopEnd = step * this.ost.pattern.length;
  }
